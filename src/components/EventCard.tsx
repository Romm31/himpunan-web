...
const EventCard: React.FC<EventCardProps> = ({ event, isPast = false }) => {
  ...
  return (
    <div ref={ref} className={`fade-in-section ${inView ? 'is-visible' : ''} h-full`}>
      <div className={`bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group h-full flex flex-col transform hover:-translate-y-2 ${isPast ? 'opacity-70 grayscale' : ''}`}>
