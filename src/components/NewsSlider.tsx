  ...
  return (
    <section id="berita" ref={ref} className={`py-20 bg-gray-50 fade-in-section ${inView ? 'is-visible' : ''}`}>
      <div className="container mx-auto px-6">
        ...
        {/* Tambahkan div pembungkus untuk custom navigation */}
        <div className="relative news-slider-container group">
          <Swiper
            modules={[Navigation, Pagination, Autoplay]}
            spaceBetween={30}
            slidesPerView={1}
            navigation={{ // Tautkan ke kelas custom
            ...
            {berita.map((item) => (
              <SwiperSlide key={item.id}>
                <Link href={`/berita/${item.id}`} className="block h-full">
                  {/* Tambahkan kelas efek glow */}
                  <div className="bg-white rounded-xl shadow-lg transition-all duration-300 overflow-hidden h-full flex flex-col card-hover-effect">
